...
# App Config.
#----------------------------------------------------------------------------#
# App configuaration is also done in a separate config.py file that uses settings.py and a .env file.



# TODO: connect to a local postgresql database
# Database connection and models are created in the models.py file separately 
...
def index():
    try:
        ...
        # For the recent artists:
        try:
            recently_listed_artists = db.session.query(Artist).order_by(
                Artist.id.desc()).limit(10).all()
        except:
            flash(f"Sorry. An error occured while fetching recently listed Artists!")

        
        
        # For the recent venues:
        try:
            recently_listed_venues = db.session.query(Venue).order_by(
                Venue.id.desc()).limit(10).all()
        except:
            flash(f"Sorry. An error occured while fetching recently listed Venues!")
        ...
        flash(f"Recently listed data could not be fetched! Please try again!",
              category="error")
        abort(500)
...
def venues():
    ...
    try:
        ...
        flash(
            f"Sorry. The venues page is not available at the moment because of an error on our end.", category="error")
        abort(500)
    finally:
...
def get_venue_result(search_term):
    try:
        ...
        # Normal search by the Venue name wihtout any filters
        data = Venue.query.filter(db.func.lower(Venue.name).like(
            f"%{search_term.lower()}%")).order_by('name').all()

        # Filter search by the State name of a Venue
        if request.form['filter_by'] == 'state':
            data = Venue.query.filter(db.func.lower(Venue.state).like(
            f"%{search_term.lower()}%")).order_by('state').all()

        # Filter search by the City name of a Venue
        elif request.form['filter_by'] == 'city':
            data = Venue.query.filter(db.func.lower(Venue.city).like(
            f"%{search_term.lower()}%")).order_by('city').all()
...
def search_venues():
    ...
    try:
        ...
        flash(f"Sorry. We couldn't fetch the results due to an error!",
              category="error")
        abort(500)
    finally:
...
def show_venue(venue_id):
    ...
    try:
        ...
        flash(
            f"The mentioned venue ID {venue_id} does not exist in our database anymore.", category="info")
        abort(404)
    finally:
        return render_template('pages/show_venue.html', venue=data)
...
def create_venue_submission():
    ...
    if form.validate():
        try:
            ...
            flash(f"Venue '{request.form['name']}' was successfully listed on Fyyur!")
            ...
            flash(
                f"'{request.form['name']}' could not be listed as a Venue!", category="error")
            db.session.rollback()
            abort(500)
        finally:
            db.session.close()
...
def get_artist_result(search_term):
    try:
        ...
        # Normal search by Artist name without any filters
        data = Artist.query.filter(db.func.lower(Artist.name).like(
            f"%{search_term.lower()}%")).order_by('name').all()

        # Filter search by the State name of an Artist 
        if request.form['filter_by'] == 'state':
            data = Artist.query.filter(db.func.lower(Artist.state).like(
            f"%{search_term.lower()}%")).order_by('state').all()

        # Filter search by the City name of an Artist 
        elif request.form['filter_by'] == 'city':
            data = Artist.query.filter(db.func.lower(Artist.city).like(
            f"%{search_term.lower()}%")).order_by('city').all()
...
def search_artists():
    ...
    try:
        ...
        flash(f"Sorry. We couldn't fetch the artist results due to an error!",
              category="error")
        abort(500)
    finally:
...
def show_artist(artist_id):
    ...
    try:
        ...
        flash(
            f"Artist with the ID {artist_id} is not listed on our database anymore.", category="info")
        abort(404)
...
def edit_artist_submission(artist_id):
    ...
    if form.validate():
        try:
            ...
            db.session.commit()
            flash(f"Artist has been updated successfully on Fyyur!")
        except:
            flash(f"Sorry. The artist information could not be updated successfully.", category="error")
            db.session.rollback()
            abort(500)
        finally:
            db.session.close()
...
def edit_venue_submission(venue_id):
    ...
    if form.validate():
        try:
            ...
            db.session.commit()
            flash(f"Venue has been successfully updated on Fyyur!")
        except:
            flash(f"Sorry. We were unable to update the venue successfully.")
            db.session.rollback()
            abort(500)
        finally:
            db.session.close()
    ...
    if form.validate():
        try:
            ...
            if not Artist.query.filter(Artist.phone == request.form['phone']).count() > 0:
                ...
                flash(
                    f"Artist with ID '{request.form['name']}' has been successfully listed on Fyyur!")
            ...
            flash(
                f"Error. Couldn't list artist successfully{' because the entered phone number exists in our database from before' if phoneExists else ''}!", category="error")
            db.session.rollback()
            abort(500)
        finally:
            db.session.close()
...
def delete_artist(artist_id):
    try:
        ...
        db.session.commit()
        flash(f"Artist with id { artist_id } has been successfully deleted from Fyyur!")
    except:
        flash(
...
def shows():
    ...
    try:
        ...
        flash(
            f"Sorry for inconveniance. We cannot load the shows page because of an issue on our end.", category="error")
        abort(500)
    finally:
...
def search_show():
    ...
    try:
        ...
        flash(
            f"Error. We couldn't fetch the search results successfully.", category="error")
        db.session.close()
        abort(500)
    finally:
...
def create_show_submission():
    ...
    try:
        ...
        db.session.commit()
        # on successful db insert, flash success
        flash('Your Show has been successfully listed on Fyyur!')
    except ValueError as e:
        # TODO: on unsuccessful db insert, flash an error instead.
        # e.g., flash('An error occurred. Show could not be listed.')
        # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/
        flash(
            f"""We weren't able to list the show {f" because the {'Artist' if not artistFound else 'Venue' if not venueFound else ''} ID entered is not present inside our database records" if not artistFound or not venueFound else f', because {e}'}!""", category="error")
        db.session.rollback()
        abort(500)
    except Exception as e:
        flash(
            f"""We weren't able to list the show {f', because {e}'}!""", category="error")
        db.session.rollback()
        abort(500)
    finally:
        db.session.close()
